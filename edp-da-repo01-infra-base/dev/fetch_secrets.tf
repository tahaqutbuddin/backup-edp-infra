...
// Fetch all the secret keys within the specified Key Vault ending with "-DEV"
...
// Filter and store only the secret keys that end with "-DEV"
locals {
  dev_secret = [
    for secret in data.azurerm_key_vault_secrets.get_all.names : secret 
    if can(regex(".*-DEV$", secret))
  ]
}

// Fetch values for the "-DEV" secrets from infra_kv
